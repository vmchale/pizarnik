...
tMs :: [FilePath] -> FilePath -> IO (Either (TE AlexPosn) (Tree (M AlexPosn (TS AlexPosn))))
tMs incls fp = do
    (u, rm) <- either throwIO pure =<< rMs incls fp
    pure $ fmap fst <$> evalStateT (tg (mempty :: Ext AlexPosn) (tr rm)) u
  where
    tg c (Node n ns) = do
        ms <- traverse (tg c) ns
...
rMs :: [FilePath] -- ^ Include dirs
    ...
    -> IO (Either (RE AlexPosn) (Int, IM.IntMap (M AlexPosn AlexPosn)))
rMs incls fp = do
    (u, MS ms ims) <- pRoot incls fp
    let s=tsort ims
    pure (go ms u IM.empty s)
  where
