...
instance P0 (T a) where
    ...
    p0 (TT _ n) = pretty n; p0 (Σ _ ts) = pΣ (pNM (\(u,tsϵ) -> if null tsϵ then pretty u else hsep (map p0 tsϵ) <+> pretty u) ts)
    p0 t@TA{} | (h:a) <- tunroll t = p0 h <> parens (hsep (punctuate "," (p0<$>a)))
