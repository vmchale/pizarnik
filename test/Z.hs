...
main = defaultMain $
    testGroup "unit"
        ...
        , testGroup "ty"
            ...
            : tE "test/examples/errorHierarchy.piz" "test/examples/errorHierarchy.piz:13:19: {`scope ⊕\n                                        `unificationFailed} ⊀ (ρ₁ ⊃ {`scope})"
            : tE "test/data/badBool.piz" "test/data/badBool.piz:4:23: failed to unify ‘{True}’ with ‘{True ⊕ False}’"
            : tE "test/data/badBool2.piz" "test/data/badBool2.piz:3:23: failed to unify ‘{True ⊕ False}’ with ‘{True}’"
